...
if ($msg) {
    ...
    echo \gc_admin_alumni_list_e($msg);
    ?></div>
    <?php 
}
?>
...
if ($error) {
    ...
    echo \gc_admin_alumni_list_e($error);
    ?></div>
    <?php 
}
?>
...
foreach ($courseOptions as $course) {
    ?>
                        <option value="<?php 
    echo \gc_admin_alumni_list_e($course);
    ?>"><?php 
    echo \gc_admin_alumni_list_e($course);
    ?></option>
                    <?php 
...
foreach ($batchOptions as $batch) {
    ?>
                        <option value="<?php 
    echo \gc_admin_alumni_list_e($batch);
    ?>"><?php 
    echo \gc_admin_alumni_list_e($batch);
    ?></option>
                    <?php 
...
foreach ($alumni as $a) {
    ...
    ?>">
                                <?php 
    echo \gc_admin_alumni_list_e($a['fullname']);
    ?>
                            </a>
                        </td>
                        <td><?php 
    echo \gc_admin_alumni_list_e($a['username']);
    ?></td>
                        <td><?php 
    echo \gc_admin_alumni_list_e($a['email'] ?? '');
    ?></td>
                        <td><?php 
    echo \gc_admin_alumni_list_e($a['course'] ?? '');
    ?></td>
                        <td><?php 
    echo \gc_admin_alumni_list_e($a['batch_year'] ?? '');
    ?></td>
                        <td><?php 
    echo \gc_admin_alumni_list_e($a['skills'] ?? '');
    ?></td>
                        <td>
...
foreach ($alumni as $a) {
    ...
    if (!empty($a['profile_picture'])) {
        ?>
                        <img src="<?php 
        echo \gc_admin_alumni_list_e(\url('') . '/uploads/profiles/' . rawurlencode($a['profile_picture']));
        ?>" alt="Profile Photo">
                    <?php 
    } else {
        ?>
                        <div class="alumni-profile-initials"><?php 
        echo \gc_admin_alumni_list_e(strtoupper(substr($a['fullname'] ?? 'A', 0, 1)));
        ?></div>
                    <?php 
    }
    ?>
    ...
    echo \gc_admin_alumni_list_e($summaryAlignment['class']);
    ?>"><?php 
    echo \gc_admin_alumni_list_e($summaryAlignment['status']);
    ?></span>
                        <div class="details-empty"><?php 
    echo \gc_admin_alumni_list_e($summaryAlignment['reason']);
    ?></div>
                    </div>
                </div>
                <div class="snapshot-item full-width"><div class="snapshot-label">Career Objective</div><div class="snapshot-value"><?php 
    echo \gc_admin_alumni_list_e($a['career_objective'] ?? '');
    ?></div></div>
    ...
    if (empty($educations)) {
        ...
        foreach ($educations as $edu) {
            ?>
                                        <tr>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($edu['school_name']);
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($edu['degree']);
            ?></td>
                                            <td><?php 
    ...
    if (empty($degrees)) {
        ...
        foreach ($degrees as $deg) {
            ?>
                                        <tr>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($deg['degree_name']);
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($deg['school_name'] ?? '');
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($deg['year_graduated'] ?? '');
            ?></td>
                                            <td>
                                                <?php 
            if (!empty($deg['diploma_file'])) {
                ?>
                                                    <a href="<?php 
                echo \gc_admin_alumni_list_e(\url('') . '/uploads/diplomas/' . rawurlencode($deg['diploma_file']));
    ...
    if ($employmentHistoryError !== '') {
        ?>
                        <div class="details-empty"><?php 
        echo \gc_admin_alumni_list_e($employmentHistoryError);
        ?></div>
                    <?php 
    } elseif (empty($jobs)) {
        ?>
        ...
        foreach ($jobs as $job) {
            ?>
                                        <tr>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['company_name']);
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['job_title']);
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['employment_type'] ?? '');
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['location'] ?? '');
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_format_date_range($job['start_date'] ?? '', $job['end_date'] ?? '');
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['job_description'] ?? '');
            ?></td>
                                            <td>
                                                <?php 
            ...
            ?>
                                                <span class="alignment-badge <?php 
            echo \gc_admin_alumni_list_e($jobAlignment['class']);
            ?>"><?php 
            echo \gc_admin_alumni_list_e($jobAlignment['status']);
            ?></span>
                                                <div class="details-empty"><?php 
            echo \gc_admin_alumni_list_e($jobAlignment['reason']);
            ?></div>
                                            </td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($job['created_at'] ?? '');
            ?></td>
                                        </tr>
                                    <?php 
        }
    ...
    if (empty($certs)) {
        ...
        foreach ($certs as $cert) {
            ?>
                                        <tr>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($cert['certificate_name']);
            ?></td>
                                            <td><?php 
            echo \gc_admin_alumni_list_e($cert['issue_date'] ?? '');
            ?></td>
                                            <td>
                                                <?php 
            if (!empty($cert['certificate_image'])) {
                ?>
                                                    <div class="certificate-preview-wrap">
                                                        <a href="<?php 
                echo \gc_admin_alumni_list_e(\url('') . '/uploads/certificates/' . rawurlencode($cert['certificate_image']));
                ?>" target="_blank">
