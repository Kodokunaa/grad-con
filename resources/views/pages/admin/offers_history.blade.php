...
if ($error !== '') {
    ...
    echo \gc_admin_offers_history_e($error);
    ?></div>
    <?php 
}
?>
...
if (empty($logs)) {
    ...
    foreach ($logs as $index => $log) {
        ...
        ?></td>
                                <td><?php 
        echo \gc_admin_offers_history_e(\gc_admin_offers_history_format_activity_date($log['created_at'] ?? ''));
        ?></td>
                                <td><?php 
        echo \gc_admin_offers_history_e($log['employer_name'] ?? 'Unknown');
        ?></td>
                                <td>
                                    <?php 
        $action = $log['action'] ?? '';
        ...
        ?>"><?php 
        echo \gc_admin_offers_history_e(str_replace('_', ' ', $action));
        ?></span>
                                </td>
                                <td>
                                    <?php 
        if (!empty($log['alumni_name'])) {
            ?>
                                        <?php 
            echo \gc_admin_offers_history_e($log['alumni_name']);
            ?>
                                        <?php 
            if (!empty($log['alumni_email'])) {
                ?>
                                            <div style="font-size:12px; color:#6b7280;"><?php 
                echo \gc_admin_offers_history_e($log['alumni_email']);
                ?></div>
                                        <?php 
            }
            ?>
