...
if (empty($archivedEvents)) {
    ...
    foreach ($archivedEvents as $event) {
        ?>
                    <?php
        [$statusText, $statusClass] = \App\Support\ViewFormatter::alumni_officer_archive_post_status_label($event['post_start_date'] ?? null, $event['post_end_date'] ?? null);
        ?>
                    <div class="event-post">
                        <div class="post-header">
                            <div class="poster-info">
                                <?php
        echo \App\Support\ViewFormatter::alumni_officer_archive_avatar_html($event['fullname'] ?? 'Unknown', 'user-avatar poster-avatar');
        ?>
                                <div>
                                    <h4 class="poster-name"><?php
        echo e($event['fullname'] ?? 'Unknown');
        ?></h4>
                                    <div class="post-meta">Archived on <?php
        echo e(! empty($event['archived_at']) ? $event['archived_at'] : 'Unknown');
        ?></div>
                                </div>
                            </div>
        ...
        echo nl2br(e($event['content'] ?? 'No description provided.'));
        ?></div>
                            <div class="schedule-line">
                                <span>🟢 Start: <?php
        echo e(! empty($event['post_start_date']) ? \App\Support\ViewFormatter::alumni_officer_archive_format_schedule_date($event['post_start_date']) : 'Immediately');
        ?></span>
                                <span>🔴 End: <?php
        echo e(! empty($event['post_end_date']) ? \App\Support\ViewFormatter::alumni_officer_archive_format_schedule_date($event['post_end_date']) : 'No end date');
        ?></span>
                            </div>
                        </div>
            ...
            foreach ($event['comments'] as $comment) {
                ?>
                                        <div class="comment-item">
                                            <?php
                echo \App\Support\ViewFormatter::alumni_officer_archive_avatar_html($comment['fullname'] ?? 'User', 'user-avatar comment-avatar');
                ?>
                                            <div class="comment-bubble">
                                                <div class="comment-top">
                                                    <div class="comment-name"><?php
                echo e($comment['fullname'] ?? 'Unknown');
                ?></div>
                                                    <div class="comment-date"><?php
                echo e(! empty($comment['created_at']) ? date('M d, Y', strtotime($comment['created_at'])) : '');
                ?></div>
                                                </div>
                                                <div class="comment-text"><?php
                echo nl2br(e($comment['comment'] ?? ''));
                ?></div>
                                            </div>
                                        </div>
                                    <?php
