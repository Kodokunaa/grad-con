...
if (! empty($applications)) {
    ...
    foreach ($applications as $row) {
        ?>
                        <?php
        $status = \App\Support\ViewFormatter::employer_applications_normalize_status($row['status'] ?? 'pending');
        ...
        ?>">
                                    <?php
        echo e(\App\Support\ViewFormatter::employer_applications_status_label($status));
        ?>
                                </span>
                            </td>
...
foreach ($applications as $row) {
    ...
    ?></div>
                ...
                <?php
    if (\App\Support\ViewFormatter::employer_applications_normalize_status($row['status'] ?? '') === 'cancelled') {
        ?>
                    <div class="snapshot-item full-width">
                        <div class="snapshot-label">Cancellation Reason</div>
                        <div class="snapshot-value">
                            <?php
        echo ! empty($row['cancel_reason']) ? e($row['cancel_reason']) : 'No reason recorded.';
        ?>
                        </div>
                    </div>
                <?php
    ...
    if (empty($educations)) {
        ...
        foreach ($educations as $edu) {
            ...
            ?></td>
                                        <td><?php
            echo \App\Support\ViewFormatter::employer_applications_format_year_range($edu['start_year'] ?? '', $edu['end_year'] ?? '');
            ?></td>
                                    </tr>
                                <?php
        }
    ...
    if (empty($jobsHist)) {
        ...
        foreach ($jobsHist as $job) {
            ...
            ?></td>
                                        <td><?php
            echo \App\Support\ViewFormatter::employer_applications_format_date_range($job['start_date'] ?? '', $job['end_date'] ?? '');
            ?></td>
                                        <td><?php
