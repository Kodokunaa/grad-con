...
if ((int)$job['is_open'] !== 1) {
    die("This job is closed and no longer accepting applications.");
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($error === "" && isset($_FILES["resume"]) && $_FILES["resume"]["error"] === UPLOAD_ERR_OK) {
        ...
        elseif ($file["size"] > $max_size) {
            ...
            $filename = "resume_job" . $job_id . "_u" . $alumni_id . "_" . time() . ".pdf";
    ...
    if ($error === "") {
        try {
            ...
            $ins->execute([
                ...
                $alumni["fullname"] ?? null,
                $alumni["email"] ?? null,
                $alumni["course"] ?? null,
                $alumni["batch_year"] ?? null,
                !empty($alumni["birthdate"]) ? $alumni["birthdate"] : null,
                !empty($alumni["age"]) ? (int)$alumni["age"] : null,
                $alumni["gender"] ?? null,
                $alumni["civil_status"] ?? null,
                $alumni["contact_number"] ?? null,
                $alumni["address"] ?? null,
                $alumni["indigenous_tribe"] ?? null,
                $alumni["special_needs"] ?? null,
                $alumni["employment_status"] ?? null,
                $alumni["job_aligned"] ?? null,
                $alumni["profile_picture"] ?? null,
                $alumni["career_objective"] ?? null,
                $alumni["skills"] ?? null,
                $alumni["trainings"] ?? null
            ]);
...
<div class="content">
    <div class="page-header">
        <h3 class="page-title">Apply for: <?php echo htmlspecialchars($job["title"]); ?></h3>
        <div class="job-meta">
    ...
    <div class="form-card">
        <div class="resume-box">
            ...
            <div class="resume-grid">
                <div class="resume-item">
                    ...
                    <div class="resume-value" id="preview-fullname"><?php echo htmlspecialchars($alumni["fullname"] ?? ""); ?></div>
                </div>

                <div class="resume-item">
                    <div class="resume-label">Email</div>
                    <div class="resume-value" id="preview-email"><?php echo htmlspecialchars($alumni["email"] ?? ""); ?></div>
                </div>

                <div class="resume-item">
                    <div class="resume-label">Course</div>
                    <div class="resume-value" id="preview-course"><?php echo htmlspecialchars($alumni["course"] ?? ""); ?></div>
                </div>

                <div class="resume-item">
                    <div class="resume-label">Batch Year</div>
                    <div class="resume-value"><?php echo htmlspecialchars($alumni["batch_year"] ?? ""); ?></div>
                </div>

                <div class="resume-item">
                    <div class="resume-label">Contact Number</div>
                    <div class="resume-value" id="preview-contact"><?php echo htmlspecialchars($alumni["contact_number"] ?? ""); ?></div>
                </div>

                <div class="resume-item">
                    <div class="resume-label">Address</div>
                    <div class="resume-value" id="preview-address"><?php echo htmlspecialchars($alumni["address"] ?? ""); ?></div>
                </div>

                <div class="resume-item full-width">
                    <div class="resume-label">Career Objective</div>
                    <div class="resume-value" id="preview-objective"><?php echo htmlspecialchars($alumni["career_objective"] ?? ""); ?></div>
                </div>

                <div class="resume-item full-width">
                    <div class="resume-label">Skills</div>
                    <div class="resume-value" id="preview-skills"><?php echo htmlspecialchars($alumni["skills"] ?? ""); ?></div>
                </div>

                <div class="resume-item full-width">
                    <div class="resume-label">Trainings / Seminars</div>
                    <div class="resume-value"><?php echo htmlspecialchars($alumni["trainings"] ?? ""); ?></div>
                </div>
            </div>
        </div>
        ...
        <form method="POST" id="applicationForm" enctype="multipart/form-data">
            ...
            <div class="actions">
                ...
                <a class="btn-outline-custom" href="<?php echo BASE_URL; ?>/alumni/jobs.php">Back</a>
            </div>
        </form>
    </div>
</div>
...
const profileData = {
    fullname: '<?php echo addslashes($alumni["fullname"] ?? ""); ?>',
    email: '<?php echo addslashes($alumni["email"] ?? ""); ?>',
    course: '<?php echo addslashes($alumni["course"] ?? ""); ?>',
    contact_number: '<?php echo addslashes($alumni["contact_number"] ?? ""); ?>',
    address: '<?php echo addslashes($alumni["address"] ?? ""); ?>',
    career_objective: '<?php echo addslashes($alumni["career_objective"] ?? ""); ?>',
    skills: '<?php echo addslashes($alumni["skills"] ?? ""); ?>'
};
