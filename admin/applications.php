...
function appColExists(array $columns, string $name): bool {
...
// ==========================
function sendAdminApplicantEmail(array $application, string $action, string $customMessage): array {
    try {
        ...
        $sender_name = $_SESSION['user']['fullname'] ?? ($_SESSION['user']['username'] ?? "Admin");
        ...
        error_log("Admin applicant email error: " . $e->getMessage());
                ...
                if (!$application) {
                    ...
                    if ($current_status === 'cancelled') {
                        ...
                        if (in_array($action, ["accept", "interview"], true)) {
                            $mailResult = sendAdminApplicantEmail($application, $action, $action_message);
...
$selectParts = [
    ...
    "a.status",
    appColExists($appColumns, 'message') ? "a.message" : "NULL AS message",
    appColExists($appColumns, 'cancel_reason') ? "a.cancel_reason" : "NULL AS cancel_reason",
    appColExists($appColumns, 'cancelled_at') ? "a.cancelled_at" : "NULL AS cancelled_at",
    appColExists($appColumns, 'created_at') ? "a.created_at" : "NULL AS created_at",
    "a.job_id",
    "u.id AS alumni_id",
    "u.fullname",
    "u.username",
...
foreach ($resumeCandidates as $col) {
    if (appColExists($appColumns, $col)) {
        $selectParts[] = "a.`$col`";
    } else {
...
<div class="content">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">
                Job: <strong><?php echo e($job['title']); ?></strong>
            </p>
            <p class="page-subtitle">
                Posted by: <strong><?php echo e($job['poster_name']); ?></strong>
                <?php if ($isEmployerPosted): ?>
    ...
    <?php if ($msg): ?>
        <div class="alert-box alert-success"><?php echo e($msg); ?></div>
    <?php endif; ?>
    ...
    <div class="section-card">
        ...
        <div class="section-subtitle">
            <?php echo $isAdminPosted ? 'View and manage applicants for this admin-posted job.' : 'View applicants for this employer-posted job.'; ?>
        </div>
                ...
                <tbody>
                    ...
                    <?php foreach ($applications as $row): ?>
                        <?php
                            ...
                            $resumeFile = '';
                            if (!empty($row['resume_file'])) $resumeFile = $row['resume_file'];
                            elseif (!empty($row['resume'])) $resumeFile = $row['resume'];
                            elseif (!empty($row['resume_path'])) $resumeFile = basename($row['resume_path']);
                            elseif (!empty($row['cv'])) $resumeFile = $row['cv'];
                            elseif (!empty($row['cv_file'])) $resumeFile = $row['cv_file'];
                            elseif (!empty($row['file'])) $resumeFile = $row['file'];
                            elseif (!empty($row['attachment'])) $resumeFile = $row['attachment'];
                        ?>
                        <tr>
                            ...
                            <td>
                                <div class="small-text" style="max-width:260px; white-space:pre-line;">
                                    <?php echo e($row['message'] ?? 'No message'); ?>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="actions" style="flex-direction: column; gap: 8px;">
                                    <?php if (!empty($resumeFile)): ?>
                                        <a href="<?php echo BASE_URL; ?>/admin/view_resume.php?app_id=<?php echo (int)$row['application_id']; ?>"
                                           class="btn"
                                    ...
                                    <?php elseif ($isAdminPosted): ?>
                                        <?php if ($status === 'pending' || $status === 'under_review'): ?>
                                            <div style="display:flex; gap:6px; flex-wrap:wrap;">
                                                ...
                                                <a href="<?php echo BASE_URL; ?>/admin/interview.php?application_id=<?php echo (int)$row['application_id']; ?>"
        ...
        <div id="snapshot-<?php echo $appId; ?>" style="display:none;">
            ...
            <div class="snapshot-grid">
                <div class="snapshot-item">
                    ...
                    <div class="snapshot-value"><?php echo e($row['email'] ?? 'N/A'); ?></div>
                </div>

                <div class="snapshot-item">
                    <div class="snapshot-label">Course / Batch</div>
                    <div class="snapshot-value">
                        <?php echo e(($row['course'] ?? 'N/A') . (!empty($row['batch_year']) ? ' / ' . $row['batch_year'] : '')); ?>
                    </div>
                </div>
                ...
                <?php if (!empty($row['cancel_reason'])): ?>
                    <div class="snapshot-item full-width" style="background:#fff7ed;border-color:#fed7aa;">
                        ...
                        <div class="snapshot-value" style="color:#7c2d12;"><?php echo e($row['cancel_reason']); ?></div>
                    </div>
                <?php endif; ?>

                <div class="snapshot-item full-width">
                    <div class="snapshot-label">Career Objective</div>
                    <div class="snapshot-value"><?php echo e($row['career_objective'] ?? 'N/A'); ?></div>
                </div>
                ...
                <div class="snapshot-item full-width">
                    ...
                    <div class="snapshot-value"><?php echo e($row['trainings'] ?? 'N/A'); ?></div>
                </div>

                <div class="snapshot-item full-width">
                    <div class="snapshot-label">Work Experience</div>
                    <div class="snapshot-value"><?php echo e($row['work_experience'] ?? 'N/A'); ?></div>
                </div>
            </div>
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="details-table">
                                ...
                                <tbody>
                                    <?php foreach ($jobsHist as $jobHist): ?>
                                        <tr>
                                            <td><?php echo e($jobHist['company_name']); ?></td>
                                            <td><?php echo e($jobHist['job_title']); ?></td>
                                            <td><?php echo e($jobHist['employment_type'] ?? ''); ?></td>
                                            <td><?php echo e($jobHist['location'] ?? ''); ?></td>
                                            <td><?php echo format_date_range($jobHist['start_date'] ?? '', $jobHist['end_date'] ?? ''); ?></td>
                                            <td><?php echo e($jobHist['job_description'] ?? ''); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
