...
function ensure_archive_column(PDO $pdo, string $column, string $definition): void {
...
try {
	ensure_archive_column($pdo, 'is_archived', 'TINYINT(1) NOT NULL DEFAULT 0');
	ensure_archive_column($pdo, 'archived_at', 'DATETIME NULL');
	...
	$archivedEvents = [];
	$error = 'Unable to load archived events: ' . $e->getMessage();
}
...
<div class="content">
	<div class="archive-wrapper">
		...
		<?php if ($message): ?><div class="alert-box alert-success"><?php echo e($message); ?></div><?php endif; ?>
		<?php if ($error): ?><div class="alert-box alert-error"><?php echo e($error); ?></div><?php endif; ?>
				...
				<article class="event-card">
					<div class="event-header">
						<div class="poster">
							...
							<div>
								<h2 class="poster-name"><?php echo e($event['poster'] ?? 'Unknown'); ?></h2>
								<div class="posted-date">Archived on <?php echo e(format_date($event['archived_at'] ?? '')); ?></div>
							</div>
						</div>
						<span class="archive-badge">Archived Event</span>
					</div>
					<div class="event-body">
						<h3 class="event-title"><?php echo e($event['title'] ?? 'Untitled event'); ?></h3>
						<div class="event-description"><?php echo e($event['content'] ?? 'No description provided.'); ?></div>
						<div class="schedule">
							<span>Start: <?php echo e(format_date($event['post_start_date'] ?? '')); ?></span>
							<span>End: <?php echo e(format_date($event['post_end_date'] ?? '')); ?></span>
						</div>
					</div>
					<?php if (!empty($event['image'])): ?>
						<img class="event-image" src="<?php echo BASE_URL; ?>/uploads/events/<?php echo e($event['image']); ?>" alt="Archived event image">
					<?php endif; ?>
					<div class="event-footer">
						<a class="restore-btn" href="<?php echo BASE_URL; ?>/admin/admin_archive.php?restore=<?php echo (int)$event['id']; ?>" onclick="return confirm('Restore this event to the active event list?');">↺ Restore Event</a>
					</div>
					<?php if (!empty($event['comments'])): ?>
						<div class="comments-section">
							<div class="comments-heading">Comments (<?php echo count($event['comments']); ?>)</div>
							<?php foreach ($event['comments'] as $comment): ?>
								<div class="comment-item">
									<div class="comment-avatar"><?php echo e(strtoupper(substr(trim($comment['fullname'] ?? 'U'), 0, 1))); ?></div>
									<div class="comment-bubble">
										<div class="comment-meta">
											<span class="comment-name"><?php echo e($comment['fullname'] ?? 'Unknown user'); ?></span>
											<span class="comment-date"><?php echo e(format_date($comment['created_at'] ?? '')); ?></span>
										</div>
