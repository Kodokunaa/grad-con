...
function short_text($text, $limit = 320): string {
...
function post_status_label($startDate, $endDate): array {
    ...
    if ($end && $end < $now) return ['Expired', 'status-expired'];
    return ['Active', 'status-active'];
}

function event_is_visible_by_schedule($startDate, $endDate): bool {
    $now = time();
...
function profile_image_url(?string $image): string {
...
function get_current_user_photo(PDO $pdo, int $userId, ?string $profileColumn): string {
    if ($userId <= 0 || !$profileColumn) return '';
    try {
...
function render_avatar(string $name, ?string $profileImage = '', string $class = 'avatar'): string {
    $url = profile_image_url($profileImage);
    if ($url !== '') {
        return '<div class="' . e($class) . '"><img src="' . e($url) . '" alt="' . e($name) . ' profile" onerror="this.style.display=\'none\'; this.parentElement.classList.add(\'avatar-fallback\'); this.parentElement.textContent=\'' . initials($name) . '\';"></div>';
    }

    return '<div class="' . e($class) . ' avatar-fallback">' . initials($name) . '</div>';
}
...
$currentFullname = $_SESSION['user']['fullname'] ?? $_SESSION['fullname'] ?? 'Admin';
$profileColumn = get_user_profile_column($pdo);
$currentUserPhoto = get_current_user_photo($pdo, $currentUserId, $profileColumn);

try {
    if (!column_exists($pdo, 'events', 'post_start_date')) {
        $pdo->exec("ALTER TABLE events ADD COLUMN post_start_date DATETIME NULL AFTER created_at");
    }
...
function get_comments(PDO $pdo, string $postType, int $postId, string $profileSelect): array {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_reaction'])) {
    ...
    $reactionResponse = [
        'success' => false,
        'message' => 'Unable to react. Please make sure you are logged in.',
    ];
    ...
    if ($eventId > 0 && $currentUserId > 0 && isset($allowedReactions[$reactionType])) {
        try {
            ...
            if (!$visibilityRow || !event_is_visible_by_schedule($visibilityRow['post_start_date'] ?? null, $visibilityRow['post_end_date'] ?? null)) {
            ...
            $reactionResponse['counts'] = get_reaction_counts($pdo, 'event', $eventId);
        } catch (Throwable $e) {
...
foreach ($events as $event) {
    ...
    $postData[$eventId] = [
        ...
        'comments' => get_comments($pdo, 'event', $eventId, $profileSelect),
    ];
}
        ...
        <section class="dashboard-hero">
            <div class="hero-row">
                ...
                <div class="hero-actions">
                    <a href="<?php echo BASE_URL; ?>/admin/events_create.php" class="btn-primary-orange">+ Create Post</a>
                    <a href="<?php echo BASE_URL; ?>/admin/dashboard.php" class="btn-light-outline">Dashboard</a>
                </div>
            </div>
        </section>
        ...
        <?php else: ?>
            <?php foreach ($events as $event): ?>
                <?php
                    ...
                    $comments = comment_total($eventComments);
                ...
                <article class="event-post" data-event-id="<?php echo $eventId; ?>" data-search="<?php echo e($searchText); ?>" data-status="<?php echo e($statusValue); ?>">
                    <div class="post-header">
                        <div class="poster-info">
                            <?php echo render_avatar($postedBy, $event['poster_profile'] ?? '', 'avatar'); ?>
                            <div>
                                <h4 class="poster-name"><?php echo e($postedBy); ?></h4>
                                <div class="post-meta">
                                    Posted an alumni event • Event ID #<?php echo $eventId; ?>
                                    <?php if (!empty($event['created_at'])): ?>
                                        • <?php echo e(format_schedule_date($event['created_at'])); ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="post-content">
                        ...
                        <div class="event-text"><?php echo nl2br(e(short_text($event["content"], 360))); ?></div>
                    ...
                    <div class="engagement-row">
                        ...
                        <button type="button" class="view-comments-btn" onclick="toggleCommentsBox('comments-box-<?php echo $eventId; ?>')"><?php echo number_format($comments); ?> Comment<?php echo $comments === 1 ? '' : 's'; ?></button>
                    </div>

                    <div class="post-actions">
                        <form method="POST" action="" class="reaction-form" data-reaction-form data-event-id="<?php echo $eventId; ?>">
                            ...
                            <div class="reaction-picker">
                                <?php foreach ($allowedReactions as $reactionKey => $reactionInfo): ?>
                                    <button type="submit" name="reaction_type" value="<?php echo e($reactionKey); ?>" class="reaction-option" title="<?php echo e($reactionInfo['label']); ?>" data-reaction="<?php echo e($reactionKey); ?>">
                                        <?php echo e($reactionInfo['emoji']); ?>
                                    </button>
                                <?php endforeach; ?>
                            </div>

                            <button type="submit" name="reaction_type" value="<?php echo e($userReaction ?: 'like'); ?>" class="btn-action btn-like reaction-main-btn <?php echo $userReaction ? 'active-' . e($userReaction) : ''; ?>" data-current-reaction="<?php echo e($userReaction ?: 'like'); ?>">
                                <span class="main-reaction-emoji"><?php echo e($reactionEmoji); ?></span> <span class="main-reaction-label"><?php echo e($reactionLabel); ?></span>
                            </button>
                        </form>

                        <button type="button" class="btn-action btn-comment" onclick="focusComment(<?php echo $eventId; ?>); showCommentsBox('comments-box-<?php echo $eventId; ?>')">💬 Comment</button>
                    ...
                    <div class="comment-section">
                        <form method="POST" action="" class="comment-form">
                            <?php echo render_avatar($currentFullname, $currentUserPhoto, 'comment-avatar'); ?>
                        ...
                        <?php if (empty($eventComments['comments'])): ?>
                        ...
                        <?php else: ?>
                            ...
                            <div class="comments-toolbar">
                                <button type="button"
                                        ...
                                        onclick="toggleCommentsBox('<?php echo e($commentsBoxId); ?>', this)"
                                        data-open-text="Hide comments"
                                        data-closed-text="View all <?php echo number_format($comments); ?> comment<?php echo $comments === 1 ? '' : 's'; ?>">
                                    View all <?php echo number_format($comments); ?> comment<?php echo $comments === 1 ? '' : 's'; ?>
                                </button>
                                <?php if ($hiddenComments > 0): ?>
                                    <span class="comment-hint"><?php echo number_format($hiddenComments); ?> hidden</span>
                                <?php endif; ?>
                            </div>

                            <div class="comments-collapse" id="<?php echo e($commentsBoxId); ?>">
                                <div class="comments-list">
                                    <?php foreach ($eventComments['comments'] as $comment): ?>
                                        <?php
                                        ...
                                        <div class="comment-thread">
                                            <div class="comment-item">
                                                <?php echo render_avatar($commentName, $comment['profile_image'] ?? '', 'comment-avatar'); ?>

                                                <div class="comment-body-wrap">
                                                    <div class="comment-bubble">
                                                        <div class="comment-top">
                                                            <div class="comment-name"><?php echo e($commentName); ?></div>
                                                    ...
                                                    <div class="comment-tools">
                                                        <button type="button" class="comment-reply-btn" onclick="toggleReplyBox('<?php echo e($replyBoxId); ?>')">Reply</button>
                                                    ...
                                                    <form method="POST" action="" class="reply-form" id="<?php echo e($replyBoxId); ?>">
                                                        <input type="hidden" name="event_id" value="<?php echo $eventId; ?>">
                                                        <input type="hidden" name="parent_comment_id" value="<?php echo $commentId; ?>">
                                                        <div class="reply-input-row">
                                                            <?php echo render_avatar($currentFullname, $currentUserPhoto, 'comment-avatar reply-avatar'); ?>
                                                            <div class="comment-input-wrap">
                                                    ...
                                                    <?php if (!empty($eventComments['replies'][$commentId])): ?>
                                                        <div class="replies-list">
                                                            <?php foreach ($eventComments['replies'][$commentId] as $reply): ?>
                                                                <?php $replyName = $reply['fullname'] ?? 'Unknown User'; ?>
                                                                <div class="comment-item reply-item">
                                                                    <?php echo render_avatar($replyName, $reply['profile_image'] ?? '', 'comment-avatar reply-avatar'); ?>

                                                                    <div class="comment-body-wrap">
                                                                        <div class="comment-bubble reply-bubble">
                                                                            <div class="comment-top">
                                                                                <div class="comment-name"><?php echo e($replyName); ?></div>
                                                                        ...
                                                                        <?php $replyReplyBoxId = 'reply-box-' . $eventId . '-' . $commentId . '-' . (int)$reply['id']; ?>
                                                                        <div class="comment-tools">
                                                                            <button type="button" class="comment-reply-btn" onclick="toggleReplyBox('<?php echo e($replyReplyBoxId); ?>', '@<?php echo e($replyName); ?> ')">Reply</button>
                                                                        ...
                                                                        <form method="POST" action="" class="reply-form" id="<?php echo e($replyReplyBoxId); ?>">
                                                                            <input type="hidden" name="event_id" value="<?php echo $eventId; ?>">
                                                                            <input type="hidden" name="parent_comment_id" value="<?php echo $commentId; ?>">
                                                                            <div class="reply-input-row">
                                                                                <?php echo render_avatar($currentFullname, $currentUserPhoto, 'comment-avatar reply-avatar'); ?>
                            ...
                            <div class="comments-preview">
                                <div class="comment-item">
                                    <?php echo render_avatar($previewCommentName, $previewComment['profile_image'] ?? '', 'comment-avatar'); ?>
                                    <div class="comment-body-wrap">
                                        <div class="comment-bubble">
                                            <div class="comment-top">
...
const allowedReactionMap = <?php echo json_encode($allowedReactions, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>;
