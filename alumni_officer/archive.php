...
function avatar_html($name, string $class = 'user-avatar'): string {
        ...
        <div class="feed-area">
            ...
            <?php else: ?>
                <?php foreach ($archivedEvents as $event): ?>
                    <?php [$statusText, $statusClass] = post_status_label($event['post_start_date'] ?? null, $event['post_end_date'] ?? null); ?>
                    <div class="event-post">
                        <div class="post-header">
                            <div class="poster-info">
                                <?php echo avatar_html($event['fullname'] ?? 'Unknown', 'user-avatar poster-avatar'); ?>
                                <div>
                                    <h4 class="poster-name"><?php echo e($event['fullname'] ?? 'Unknown'); ?></h4>
                                    <div class="post-meta">Archived on <?php echo e(!empty($event['archived_at']) ? $event['archived_at'] : 'Unknown'); ?></div>
                                </div>
                            </div>
                        ...
                        <div class="post-content">
                            <h2 class="event-title"><?php echo e($event['title'] ?? 'Untitled event'); ?></h2>
                            <div class="event-text"><?php echo nl2br(e($event['content'] ?? 'No description provided.')); ?></div>
                        ...
                        <?php if (!empty($event['image'])): ?>
                            <div class="event-image-wrap">
                                <img src="<?php echo BASE_URL; ?>/uploads/events/<?php echo e($event['image']); ?>" class="event-image" alt="Archived event image" onclick="openImageLightbox(this.src)">
                            </div>
                        <?php else: ?>
                            <div class="event-image-wrap">
                        ...
                        <div class="engagement-row">
                            ...
                            <div><?php echo count($event['comments'] ?? []); ?> Comment<?php echo count($event['comments'] ?? []) === 1 ? '' : 's'; ?></div>
                        </div>

                        <div class="post-actions">
                            <a href="<?php echo BASE_URL; ?>/alumni_officer/archive.php?restore=<?php echo (int)$event['id']; ?>" class="btn-action btn-edit">↺ Restore</a>
                        </div>
                                ...
                                <div class="comments-list">
                                    <?php foreach ($event['comments'] as $comment): ?>
                                        <div class="comment-item">
                                            <?php echo avatar_html($comment['fullname'] ?? 'User', 'user-avatar comment-avatar'); ?>
                                            <div class="comment-bubble">
                                                <div class="comment-top">
                                                    <div class="comment-name"><?php echo e($comment['fullname'] ?? 'Unknown'); ?></div>
                                                    <div class="comment-date"><?php echo e(!empty($comment['created_at']) ? date('M d, Y', strtotime($comment['created_at'])) : ''); ?></div>
                                                </div>
