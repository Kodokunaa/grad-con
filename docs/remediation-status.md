...
|---|---|---|---|
| Critical architecture | In progress | Native authentication; native redirects; Eloquent event, training, job, approval, and employer-offer workflows | 29 remaining PDO page controllers, `PageContext`, `PageResponse`, global helpers, and `page-functions.php` |
...
| Routing | In progress | GET-only read pages and compatibility redirects; named application resume delivery, training/job/certificate/employment deletion, event archive/restore, and offer-response actions; no remaining GET mutation handlers; method regression tests | Split remaining multi-action POST pages into named resource actions; retire `.php` compatibility URLs after frontend links are canonical |
...
| Authorization | In progress | Policies for jobs, applications, offers, interviews, events, training, certificates, employment history, private files, and policy-enforced administrative alumni editing | Enforce policies in every retained mutation controller and add record-level tests for each workflow |
| Validation | In progress | Auth validation plus Form Requests for administrative employer, officer, alumni creation/editing, resume forwarding, events, training, jobs, approvals, and job notifications | Add Form Requests for jobs, remaining application actions, offers, interviews, profiles, education, employment, and social actions |
...
| Mail and queues | In progress | Laravel queued delivery; reset notification; dedicated disk-backed applicant-resume and training-opportunity Mailables; configured sender addresses; removal of PHPMailer transport setup | Application/interview compatibility helpers remain; remove `PageMailer` after replacing them; document/verify worker and SMTP deployment |
...
| Middleware and responses | In progress | Native form CSRF, native page assets/modal, no response-body HTML rewriting or query-action interception, non-fatal audit records, upload validation, security headers | Move audit recording to a service or listener; move broad upload/password checks into Form Requests |
...
- Destructive GET query interception has been removed in favor of explicit resource routes and confirmation components.
