...
| Routing | In progress | GET-only read pages and compatibility redirects; named application resume-delivery and training-deletion actions; method regression tests | Split remaining multi-action pages into named POST/PATCH/DELETE resource actions; remove destructive query parameters and `.php` URLs |
