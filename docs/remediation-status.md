...
| Routing | In progress | GET-only read pages and compatibility redirects; named application resume delivery, training/job/certificate/employment deletion, and event restoration actions; method regression tests | Split remaining multi-action pages into named POST/PATCH/DELETE resource actions; remove remaining destructive query parameters and `.php` URLs |
